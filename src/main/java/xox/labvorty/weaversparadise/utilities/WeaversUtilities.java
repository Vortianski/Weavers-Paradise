...
public class WeaversUtilities {
    ...
    public static boolean isRestricted(LivingEntity entity, List<? extends String> restrictorIds, EquipmentSlot... slotsToCheck) {
        if (restrictorIds.isEmpty()) return false;

        EquipmentSlot[] slots = slotsToCheck.length > 0 ? slotsToCheck : ARMOR_SLOTS;

        for (EquipmentSlot slot : slots) {
            ItemStack stack = entity.getItemBySlot(slot);
            if (!stack.isEmpty() && ClientConfig.containsItem(restrictorIds, stack.getItem())) {
                boolean restricted = evaluateRestrictor(stack, entity, slots);
                if (restricted) return true;
            }
        }

        return checkCurios(entity, restrictorIds, slots);
    }

    private static boolean checkCurios(LivingEntity entity, List<? extends String> restrictorIds, EquipmentSlot[] slots) {
        for (Tuple<SlotReference, ItemStack> pair : WPTrinkets.getAllEquipped(entity)) {
    ...
    private static boolean evaluateRestrictor(ItemStack restrictorStack, LivingEntity entity, EquipmentSlot[] slots) {
        if (!(restrictorStack.getItem() instanceof ModelReplacer)) {
            return true;
        }

        if (ClientConfig.FULL_ARMOR.get()) {
            return true;
        }

        for (EquipmentSlot slot : slots) {
            ItemStack slotStack = entity.getItemBySlot(slot);
            boolean isOwnClothing = slotStack.getItem() instanceof SingleSidedClothingItem
                    || slotStack.getItem() instanceof DoubleSidedClothingItem;

            if (!slotStack.isEmpty() && !isOwnClothing) {
                return true;
            }
        }
    ...
    public static ItemStack createRandomSingleSidedClothing(SingleSidedClothingItem item) {
        ...
        WPUtilities.serializeSpecificVector3f(
                tag,
    ...
    public static ItemStack createRandomDoubleSidedPlushie(DoubleSidedBlockItem item) {
        ItemStack stack = new ItemStack(item);
