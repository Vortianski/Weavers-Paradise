...
public class ClothcraftingJEICategory implements IRecipeCategory<ClothcraftingJEIRecipe> {
    ...
    public ClothcraftingJEICategory(IGuiHelper guiHelper) {
        icon = guiHelper.createDrawableItemStack(new ItemStack(WeaversParadiseItems.CLOTHCRAFTING_STATION.get()));
    }
    ...
    public void draw(ClothcraftingJEIRecipe recipe, @NotNull IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
        guiGraphics.blit(ResourceLocation.fromNamespaceAndPath("weaversparadise", "textures/recipes/clothcrafting_recipe_support.png"), 0, 0, 0, 0, 192, 118, 192, 118);

        int max = recipe.getMaxScore();
        String scoreText = Component.translatable("weaversparadise.clothcrafting.score").getString() + " " + recipe.getMinScore() + "-" + (max >= 999 ? "∞" : max);
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, ClothcraftingJEIRecipe recipe, @NotNull IFocusGroup focuses) {
        ...
        builder.addOutputSlot(120, 101).addItemStack(recipe.getSpoolReturn());
    }
}