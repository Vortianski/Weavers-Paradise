...
public class BottledDyeItem extends Item {
    ...
    public void appendHoverText(@NotNull ItemStack itemStack, @NotNull TooltipContext tooltipContext, @NotNull List<Component> tooltipComponents, @NotNull TooltipFlag tooltipFlag) {
        ...
        if (tooltipFlag.isAdvanced() || tooltipFlag.hasShiftDown()) {
            tooltipComponents.add(Component.literal("Main color: " + getItemMainColor(itemStack)).withStyle(style -> style.withColor(ChatFormatting.DARK_GRAY)));
            tooltipComponents.add(Component.literal("Secondary color: " + getItemSecondaryColor(itemStack)).withStyle(style -> style.withColor(ChatFormatting.DARK_GRAY)));
        }
    }
}
