...
public class ThighHighsSilk extends Item implements ICurioItem, ThighHighsInterface {
    private static final ResourceKey<Enchantment> enchantment = ResourceKey.create(Registries.ENCHANTMENT, ResourceLocation.fromNamespaceAndPath("weaversparadise", "water_strider"));
    public ThighHighsSilk() {
    ...
    public boolean supportsEnchantment(ItemStack stack, Holder<Enchantment> enchantment) {
        return enchantment.is(Enchantments.UNBREAKING) || enchantment.is(Enchantments.VANISHING_CURSE) || enchantment.is(Enchantments.MENDING) || enchantment.is(Enchantments.BINDING_CURSE) || enchantment.is(WeaversParadiseEnchantments.WATER_STRIDER);
    }

    @Override
    public boolean isBookEnchantable(ItemStack stack, ItemStack book) {
        return EnchantmentHelper.getEnchantmentsForCrafting(book).keySet().stream().anyMatch(holder -> {
            if (holder.is(Enchantments.MENDING) || holder.is(Enchantments.UNBREAKING) || holder.is(Enchantments.VANISHING_CURSE) || holder.is(Enchantments.BINDING_CURSE) || holder.is(WeaversParadiseEnchantments.WATER_STRIDER)) {
                return true;
            }
            return false;
        });
