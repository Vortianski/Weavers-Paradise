...
public class IntersexFlagItem extends HangingEntityItem {
    private final EntityType<? extends HangingEntity> type;

    public IntersexFlagItem() {
        super(
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (player != null && !this.mayPlace(player, direction, itemstack, blockpos1)) {
            ...
            if (this.type == WeaversParadiseEntityTypes.HANGING_FLAG_ENTITY.get()) {
                Optional<HangingFlagEntity> hangingFlagEntity = HangingFlagEntity.create(level, blockpos1, direction, "intersex");
