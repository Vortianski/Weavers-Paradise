...
public abstract class DoubleSidedClothingItem extends Item implements ICurioItem {
    ...
    @Override
    public boolean canApplyAtEnchantingTable(@NotNull ItemStack itemStack, Enchantment enchantment) {
        return getSupportedEnchantments().contains(enchantment);
    }

    @Override
    public boolean isBookEnchantable(@NotNull ItemStack itemStack, @NotNull ItemStack bookStack) {
        return bookStack.getAllEnchantments().keySet().stream().allMatch((holder ->
                getSupportedEnchantments().contains(holder)
        ));
    }
